ssum <- function(x)  {sum(x[is.finite(x)]) } # ssum allows negative scale factors in renyi function
renyi2 <-  function (x, scales = c(-65, -32, -16, -8, -4, -2, -1, -0.5,-0.25,0, 0.25, 0.5, 1, 2, 4, 8, 16, 32, 64,Inf), hill = FALSE) 
{
    x <- as.matrix(x)
    n <- nrow(x)
    p <- ncol(x)
    ...
    for (a in 1:m) {
        if (scales[a] != 0 && scales[a] != 1 && scales[a] != 
            Inf) {
            result[, a] <- log(apply(x^scales[a], 1, ssum))/(1 - 
                scales[a])
        }
        else {
            if (scales[a] == 0) {
